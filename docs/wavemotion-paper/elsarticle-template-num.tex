...
\subsection{Comparison against the ideal cloak}

We compare three configurations: the raw per-cell baseline,
neural-field reparameterisation~\eqref{eq:neural_relative}, and the
continuous $\mbf{c}_{\mathrm{eff}}$ reference, all using the $14\times10$
grid. The 4 Cauchy moduli per cell are initialised at a starting point
($\mathcal{D}_{\mathrm{r}}=\textbf{[FILL: confirm D\_r for 14$\times$10 grid]}\%$)
and then freely optimised. % TODO confirm init
The right-boundary distortion at convergence is reported in
Table~\ref{tab:opt_results}.

\begin{table}[htbp]
\centering
\caption{Single-frequency cloaking distortion at $f^\star=2.0$
relative to the $14\times10$ Cartesian cloak (4CM, four free orthotropic
Cauchy moduli). The neural-field run reaches the same transmission-side
distortion as the per-cell baseline with $\sim$4$\times$ fewer parameters
and without any explicit smoothness penalty.}
\label{tab:opt_results}
\begin{tabular*}{\textwidth}{@{}llll@{}}
\toprule
Configuration & $\mathcal{D}_{\mathrm{r}}$ [\%]
              & $\mathcal{D}_{\partial}$ [\%]
              & $\mathcal{D}_{\mathrm{out}}$ [\%] \\
\midrule
Uncoated defect                            & 70.31 & ---  & 5.18 \\
$14\times10$ initialisation % TODO confirm D_r
                                           & \textbf{[FILL]} & --- & --- \\
Continuous $\mbf{c}_{\mathrm{eff}}$ (ideal polar) & 27.51 & 1.33 & 1.57 \\
\midrule
Per-cell optimised 4CM (Adam)              & \textbf{[FILL]} & \textbf{[FILL]} & \textbf{[FILL]} \\
Neural-field optimised 4CM                 & \textbf{[FILL]} & \textbf{[FILL]} & \textbf{[FILL]} \\
\bottomrule
\end{tabular*}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{convergence_comparison.pdf}
  \caption{Right-boundary distortion $\mathcal{D}_{\mathrm{r}}$ during
  optimisation for the per-cell (Adam) and neural-field
  reparameterisation on the $14\times10$ grid. Both methods drop below
  the continuous $\mbf{c}_{\mathrm{eff}}$ baseline
  ($\textbf{[FILL]}\%$) within $\sim\!10$ iterations and reach
  $\textbf{[FILL]}\%$ by step~50. Dashed/dotted lines mark the
  $14\times10$ initialisation and the continuous
  transformation-elasticity limit.}
  \label{fig:convergence_comparison}
\end{figure}

% \subsection{Mesh-refinement convergence}\label{subsec:mesh_conv}

% To isolate the residual FEM discretisation error we run the
% continuous cloak on a sequence of base meshes
% $N_e\in\{6,12,25,50,100,200\}\,\text{k}$ triangular elements with
% the same distance-based refinement schedule. Figure~\ref{fig:mesh_conv}
% reports the right-boundary and outside-cloak distortions as functions
% of $N_e^{-1/2}$ (proxy for a characteristic element length $h$). The
% linear fit on the four finest meshes extrapolates to
% $\mathcal{D}_{\mathrm{r},\infty}\approx 19\pm 5\%$ and
% $\mathcal{D}_{\mathrm{out},\infty}\approx 0.75\%$; the residual
% right-boundary distortion of the
% ideal cloak is therefore not an optimisation artefact but a finite-mesh
% artefact of the corner singularities at the notch tip and the cloak
% vertices.

% \begin{figure}[htbp]
%   \centering
%   \includegraphics[width=0.8\linewidth]{mesh_convergence.png}
%   \caption{FEM-mesh convergence of the continuous
%   $\mbf{c}_{\mathrm{eff}}$ cloak. Right-boundary distortion
%   $\mathcal{D}_{\mathrm{r}}$ and outside-cloak distortion
%   $\mathcal{D}_{\mathrm{out}}$ are plotted against $N_e^{-1/2}$;
%   dashed lines indicate power-law fits extrapolated to $h\to 0$.}
%   \label{fig:mesh_conv}
% \end{figure}

\subsection{Wavefield and material visualisations}\label{subsec:wavefield}

Beyond the scalar distortion metrics, three visual diagnostics are
reported for each single-frequency design. Figure~\ref{fig:reu_comparison}
overlays the real part of the displacement field
$\operatorname{Re}[\mbf{u}(x,y)]$ for the uncoated defect, the analytical
continuous cloak, and the optimised neural-field cloak against the
reference (defect-free) solution; the transmission-side reconstruction is the
visual analogue of the right-boundary metric. Figure~\ref{fig:unorm}
reports the magnitude $\|\mbf{u}\|$, which is invariant under the
phase ambiguity inherent to frequency-domain solutions and exposes
residual scattering inside the physical domain that is not captured by
the boundary $L^2$. Figure~\ref{fig:matheatmap} visualises the per-cell
material distribution
$(C_{11},C_{12},C_{22},C_{66},\rho)$ of the optimised cloak, providing
the basis for the symmetry and parameterisation ablations of
Sections~\ref{subsec:flat4} and~\ref{subsec:symmetrised}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{reu_comparison.png}
  \caption{Real part of the displacement field at $f^\star=2.0$.
  Columns: uncoated defect, continuous $\mbf{c}_{\mathrm{eff}}$,
  neural-field optimised cloak, reference (defect-free) solution. The
  optimised cloak reconstructs the reference field on the transmission
  side while preserving the wavefront topology of the upstream field.}
  \label{fig:reu_comparison}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{u_norm_comparison.png}
  \caption{Displacement magnitude $\|\mbf{u}\|$ at $f^\star=2.0$
  for the same four configurations as Fig.~\ref{fig:reu_comparison}.
  The magnitude visualisation suppresses the phase ambiguity and
  highlights residual scattering pockets inside the physical domain
  that the boundary metric $\mathcal{D}_{\mathrm{r}}$ averages out.}
  \label{fig:unorm}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{material_heatmap.png}
  \caption{Per-cell material distribution of the optimised
  $14\times10$ neural-field cloak, showing the four free Cauchy
  moduli $(C_{11},C_{12},C_{22},C_{66})$ and the density $\rho$.
  Black contours trace the inner (defect) and outer cloak boundaries.}
  \label{fig:matheatmap}
\end{figure}


\subsection{Symmetrised-tensor reference}\label{subsec:symmetrised}

The arithmetic-mean symmetrisation of \citet{chatzopoulos2023} provides a
closed-form reference point in the realisable Cauchy design family. The ideal
polar tensor~\eqref{eq:Ceff} is symmetrised analytically by averaging each
broken minor-symmetry pair, instantiated on the Cartesian cell grid, and run
through the same FEM solver as the optimised designs. Note that this
closed-form symmetrised tensor is a 6-component anisotropic Cauchy material,
whereas the 4CM parameterisation used in this work is the narrower
4-component orthotropic Cauchy class ($C_{16}=C_{26}=0$); the two are
different Cauchy materials. Our 4CM cloak is obtained by an independent
numerical search over the orthotropic Cauchy moduli — a different route to a
realisable design — and can therefore match or improve on the closed-form
symmetrised point within the same realisable class.
Figure~\ref{fig:nassar_repro} compares the symmetrised reference and the
4CM optimum at the design frequency.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{nassar_triangular_reproduction.png}
  \caption{Arithmetic-mean symmetrised reference (Chatzopoulos 2023,
  closed-form Cauchy baseline) reconstructed on the Cartesian cell grid
  (left), 4CM optimum from direct numerical search (centre), and
  10-PT ideal polar reference (right, unrealisable BGM bound) at
  $f^\star=2.0$. The symmetrised tensor is one specific point in the
  realisable Cauchy family; the 4CM optimum is found by an independent
  numerical search over the orthotropic Cauchy moduli.}
  \label{fig:nassar_repro}
\end{figure}

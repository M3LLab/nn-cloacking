...
Physical realizability is addressed using a database of homogenized microstructures through conditional diffusion, neural-field inverse design, and nearest-neighbour selection. 
FEM simulations show that the unconstrained design approaches ideal transformation-based cloaking, 
while the best realizable design retains approximately 97\% of this performance, providing a direct route from continuum optimization to microstructured Rayleigh-wave cloaks.
...
The triangular cloak is, however, especially attractive because its piecewise-affine transformation 
has a constant Jacobian within each half of the cloak and consequently, the required effective 
material properties are homogeneous in each region.

Our approach to find a design inside the Cauchy class of ordinary
...
Section~\ref{sec:single_freq} reports single-frequency results against the
ideal and symmetrised baselines. Section~\ref{sec:multifreq} extends the
...
The design objective is the magnitude-band
integral~\eqref{eq:opt_loss_cloak}: the mean-squared relative error of
the displacement magnitude over a near-surface strip beyond the cloak, 
which optimisation minimises. To report performance we also
use the dimensionless cloak ratio
$\eta=\langle|\mbf{u}|\rangle/\langle|\mbf{u}_{\mathrm{ref}}|\rangle$ over
the free surface beyond the cloak footprint, following
\citet{chatzopoulos2023}; it is an interpretable evaluation metric, with
$\eta\to1$ for a perfect cloak.
...
the microstructure realisability -- MLP can drive
...
and solved with the same FEM.

It is worth being precise about what that baseline actually is, because it is
richer than the $1\times1$ tile it is compared against. Symmetrisation
\emph{averages} each broken pair of Sec.~\ref{subsec:triangular} to its mean
rather than annihilating it, so the coupling entries $c_{11,12}$ and
$c_{22,12}$ survive: the symmetrised medium retains normal--shear coupling and
is a \emph{fully anisotropic} Cauchy material --- six free moduli
$(C_{11},C_{22},C_{66},C_{12},C_{16},C_{26})$ and a density, which we label
6CM --- rather than the orthotropic form~\eqref{eq:4cm}. Nor is it one
material: as noted in Sec.~\ref{subsec:triangular}, $F_{21}$ merely changes
sign across the centreline, so the ideal tensor~\eqref{eq:Ceff} and its
symmetrisation alike consist of two mirror-image phases. The single 4CM tile
therefore competes against symmetrisation from strictly \emph{less} material
freedom: one orthotropic tensor against two anisotropic ones.
...
$\eta=0.86$ --- a substantial jump towards the ideal continuous polar cloak
($\eta=0.999$) obtained from just one degree of material freedom per phase,
from a smaller material class and half the materials, and clear evidence that
optimising inside the Cauchy class outperforms symmetrisation. It is, however,
still far from perfect:
one material most certainly cannot reproduce the cloaking of the ideal tensor.
...
That the two halves of the ideal cloak are mirror images also pins down
exactly what a second material must supply, and why one tile saturates near
$\eta=0.86$ for structural rather than numerical reasons. Evaluating the two
halves shows that the mirror acts \emph{only} on the coupling: symmetrised,
both share $(C_{11},C_{22},C_{66},C_{12})=(648,293,56,403)\,$MPa and
$\rho=2400\,$kg\,m$^{-3}$, and differ solely in
$(C_{16},C_{26})=(\pm162,\pm110)\,$MPa. The orthotropic class~\eqref{eq:4cm}
sets $C_{16}=C_{26}=0$ identically, so both ideal halves collapse onto the
\emph{same} 4CM tensor: within 4CM a left--right split offers no freedom the
ideal cloak actually uses, and one tile is already the best a 4CM design can
do with such a split.

Matching the symmetrisation baseline in both respects --- its own 6CM class,
and one tile per half --- makes the comparison like-for-like and closes much
of the remaining gap: two optimised 6CM tiles, one on each side of $x_c$,
reach $\eta=0.947$ against symmetrisation's $0.634$ on the same mesh, source,
loss and design frequency (Table~\ref{tab:single_vs_sym}). Given identical
material freedom, gradient-based design thus recovers roughly two thirds of
the gap that hand symmetrisation leaves. The optimiser also
rediscovers the cloak's mirror structure unprompted: from an isotropic seed it
converges to tensors that are mirror images to within $3\%$, and constraining
the pair to be exactly mirror-symmetric costs only $0.002$ in $\eta$
($0.944$ versus $0.947$) at half the design variables. The small residual
asymmetry reflects the one-sided incident wave, which breaks the symmetry of
the scattering problem that the geometry itself possesses.

Two caveats bound this result. First, $\eta=0.947$ is a \emph{continuum-level}
statement: the optimised tensors are genuinely non-orthotropic --- no rotation
brings both $C_{16}$ and $C_{26}$ to zero (the residual coupling remains
$10$--$34\%$ of $\sqrt{C_{11}C_{66}}$) --- so they cannot be realised by the
$D_2$-symmetric square cells of Sec.~\ref{sec:micro}, whose edge mirrors force
$C_{16}=C_{26}=0$, nor by rotating such cells. This limitation is shared by
the symmetrisation baseline, which is 6CM for the same reason, and it is what
motivates restricting the remainder of the paper to the 4CM class that the
microstructure catalogue can actually build. Second, the optimum is not the
transformation cloak: it stiffens the shear modulus $8\times$ and the density
$1.7\times$ relative to the ideal tensor while halving $C_{12}$, consistent
with the symmetrised ideal itself reaching only $\eta=0.63$.
...
\begin{figure}[htbp]
  ...
  optimised material recovers $\eta=0.86$ against $0.63$ for symmetrisation.}
...
automaton on a solid/void substrate, following the cellular-automaton
...
under neighbour-count birth/death/survival rules.
...
implies that a mesh-converged, pixel-resolved solve of the full cloak would
reproduce the homogenised cloak ratio to within
...
relevant metric. We therefore validate at the unit-cell level, where each
generated cell reproduces its target stiffness under the same periodic-FEM
homogenisation used to build the catalogue.
...
\paragraph{Limitations.} The cloak is discretised into square cells, each
carrying a single homogenised material, so the piecewise-constant grid cannot
follow arbitrary cloak interfaces exactly, and the microstructure pipeline
is restricted to a two-phase (single-constituent and void) substrate described
by a quasi-static homogenised stiffness. We also validate the realised cloak
one cell at a time rather than by simulating the fully assembled
microstructure: the cells are so fine relative to the wavelength that meshing
every feature across the whole cloak is impractical, while that same scale
separation is what makes the homogenised description reliable in the first
place. A direct full-domain pixel simulation is left to future work
(Sec.~\ref{sec:micro}).
...
the realizability side, we have already replaced the discrete database
with a differentiable generative model of the realisable manifold,
queried during optimisation (a guided diffusion model in the spirit
of~\citet{yang2024}); this model still generates one unit cell at a
...
open problem that may again be a good fit for diffusion models. Such a
generator would also relax the square-cell grid used here, which cannot
conform to the inclined cloak interfaces: laying cells out on a
body-fitted mesh would let the microstructure follow the interfaces and
remove a source of residual boundary distortion. Two further extensions
...
-- to our knowledge the first microstructured realisation of a
Rayleigh-wave elastodynamic carpet cloak from a common material --

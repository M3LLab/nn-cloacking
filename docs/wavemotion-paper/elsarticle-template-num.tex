...
\author[1]{David Aznaurov}
\author[1]{David Piliposyan}
\author[1]{Danila Rukhovich}
\author[2]{Sébastien Guenneau}

\affiliation[1]{organization={M:3L Lab, Institute of Mechanics},
                country={Armenia}}

\affiliation[2]{organization={The Blackett Laboratory, Department of Physics, Imperial College London},
                addressline={London SW7 2AZ},
                country={United Kingdom}}

\begin{abstract}

Transformation elasticity provides exact material distributions for elastodynamic cloaks, but the required stiffness tensors generally violate the minor symmetries of classical Cauchy elasticity and are difficult to realise using conventional materials.
...
Here we formulate 2D Rayleigh-wave carpet-cloak design as a PDE-constrained optimisation problem, using a coordinate-based neural network (neural-field) and a differentiable finite-element solver to optimise symmetric stiffness and density fields by minimising wave-field distortion.
...
while the best realisable design retains approximately 97\% of this performance, providing a direct route from continuum optimisation to microstructured Rayleigh-wave cloaks.
...
to a local frame. The grid resolution is itself a design choice and is
selected in Sec.~\ref{subsec:grid_selection}.
...
through the differentiable FEM.
...
The multiplicative residual~\eqref{eq:neural_relative} does not by itself guarantee 
...
optimising inside the Cauchy class outperforms symmetrisation.
...
ratio to $\eta=0.962$, i.e.\ within $4\%$ of a perfect cloak. The improvement
is markedly anisotropic: subdividing in depth helps far more than in the
propagation direction ($1\times2$ reaches $\eta=0.92$ whereas $2\times1$
actually drops to $\eta=0.78$), reflecting that the ideal cloak material
varies mainly with depth through the shear map~\eqref{eq:shearmap}. Beyond
four materials the gain per added cell is small, motivating a more systematic
grid study.
...
cell, with the solid pixels carrying the concrete moduli above and the void
pixels an ersatz stiffness $10^{-6}E$, yields its effective orthotropic
stiffness $(C_{11},C_{22},C_{12},C_{66})$ and density
$\rho_{\mathrm{eff}}=v_f\rho_{\mathrm{concrete}}$. Because the squared-assembly cells are
...
\paragraph{Step 3 -- Realisability prior.} A Gaussian mixture is fitted to the
...
TODO: Add pixel-level validation details.

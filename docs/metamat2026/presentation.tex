...
\begin{frame}{Hiding from a surface wave}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      and the triangular cloak over the inclusion.}
    \end{column}
  \end{columns}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{ML methods for realisable cloaks}
...
\begin{frame}{Neural field}
  \begin{columns}[T]
    ...
    \begin{column}{0.55\textwidth}
      ...
      (Sitzmann et al., 2020). All fit the image (top); only some reproduce its
      \textbf{gradients} and \textbf{Laplacian} -- plain ReLU has none.}
    \end{column}
  \end{columns}
...
% ------------------------------------------------------------
%  SLIDE 8b -- Assembly rule: chiral vs squared
% ------------------------------------------------------------
\begin{frame}{Design space: chiral vs squared cells}
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      \centering
      {\small\textbf{Squared: mirror} ($D_4$)}\\[0.3em]
      \begin{tikzpicture}[scale=0.62, line width=0.7pt]
        \draw[ASTRAnavy] (-1.2,-1.2) rectangle (1.2,1.2);
        \draw[ASTRAnavy!30] (0,-1.2) -- (0,1.2);
        \draw[ASTRAnavy!30] (-1.2,0) -- (1.2,0);
        % mirror lines
        \draw[dashed, ASTRAorange, thick] (0,-1.5) -- (0,1.5);
        \draw[dashed, ASTRAorange, thick] (-1.5,0) -- (1.5,0);
        % double-headed reflection arrows across each mirror line
        \draw[Latex-Latex, ASTRAorange, thick] (-0.75,0.6) -- (0.75,0.6);
        \draw[Latex-Latex, ASTRAorange, thick] (-0.6,-0.75) -- (-0.6,0.75);
        \node[font=\scriptsize, ASTRAnavy] at (-0.85,0.9) {Q};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering
      {\small\textbf{Chiral: rotate} ($C_4$)}\\[0.3em]
      \begin{tikzpicture}[scale=0.62, line width=0.7pt]
        \draw[ASTRAnavy] (-1.2,-1.2) rectangle (1.2,1.2);
        \draw[ASTRAnavy!30] (0,-1.2) -- (0,1.2);
        \draw[ASTRAnavy!30] (-1.2,0) -- (1.2,0);
        % clockwise 90-degree rotation arrows between quadrants
        \draw[-Latex, ASTRAorange, thick] (-0.6,0.95) arc (135:45:0.85);
        \draw[-Latex, ASTRAorange, thick] (0.95,0.6) arc (45:-45:0.85);
        \draw[-Latex, ASTRAorange, thick] (0.6,-0.95) arc (-45:-135:0.85);
        \draw[-Latex, ASTRAorange, thick] (-0.95,-0.6) arc (225:135:0.85);
        \node[font=\scriptsize, ASTRAnavy] at (-0.6,0.6) {Q};
        \node[font=\scriptsize, ASTRAorange] at (0.62,0.62) {$90^\circ$};
      \end{tikzpicture}
    \end{column}
  \end{columns}
  \vspace{-0.4em}
  \begin{center}
    \includegraphics[width=0.45\linewidth]{figures/chiral_vs_squared_cells.png}\\
    {\scriptsize Real $50\times50$ cells, the same quadrant Q both
    ways~\cite{nakarmi2024}.}
  \end{center}
\end{frame}
...
\begin{frame}{Results I -- one material already beats symmetrisation}
  \begin{columns}[T]
    ...
    \begin{column}{0.55\textwidth}
      ...
      {\scriptsize Top$\to$bottom: reference (no obstacle), ideal polar cloak,
...
\begin{frame}{Results I -- the cloak in motion}
  \centering
  \animategraphics[width=0.9\linewidth, autoplay, loop, controls, poster=first]%
  ...
  \takeaway{The wavefront leaves the cloak intact -- no shadow, no reflected
    front.}
\end{frame}

"""Run the trained 2D conditional diffusion model on 6 dataset conditions and
...
def main() -> None:
    ...
    parser.add_argument("--output", required=True, help="Output figure path (.png/.pdf)")
    args = parser.parse_args()

    device = args.device if (args.device == "cpu" or torch.cuda.is_available()) else "cpu"
    torch.manual_seed(args.seed)

    # Use the same split logic as training so val indices match.
    ...
    fig.suptitle(
        f"Conditional diffusion samples (steps={args.steps}, w={args.tensor_w})",
        fontsize=12,
    )
